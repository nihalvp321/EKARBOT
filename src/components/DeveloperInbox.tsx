...
const DeveloperInbox = () => {
  ...
  const getContactIcon = (type: string) => (type === 'user_manager' ? Shield : User);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, data]); // 🟢 Update message state immediately
      setNewMessage('');
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.error('File message failed');
        return;
      }

      setMessages(prev => [...prev, insertedMessage]); // 🟢 Add file message instantly
  ...
  const uploadVoiceMessage = async (audioBlob: Blob) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, voiceMessage]); // 🟢 Add voice message
      toast.success('Voice message sent');
    } catch (error) {
      console.error('Voice message error:', error);
      toast.error('Voice upload error');
    } finally {
      setUploading(false);
    }
  };
                          ...
                          <div className="space-y-2">
                            {message.attachment_type?.startsWith('image/') ? (
                              ...
                              <div className="flex items-center space-x-2">
                                ...
                                <a href={message.attachment_url} target="_blank" rel="noreferrer" className="underline">
              ...
              <div className="bg-white border-t p-4">
                <div className="flex space-x-2">
                  ...
                  <Button
                    ...
                    className="bg-gray-700 text-white hover:opacity-90"
                  >
